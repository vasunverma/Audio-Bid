...
class JobFilter(django_filters.FilterSet):
    ...
    created_date = django_filters.DateFilter(widget=forms.DateInput(attrs={'placeholder':'Created Date', 
                                                                           'class':'form-control', 
                                                                           'type': 'date',
    ...
    end_date = django_filters.DateFilter(widget=forms.DateInput(attrs={'class':'form-control', 
                                                                       'type':'date',
                                                                       'style':'margin-bottom: 1rem;font-size: 15px;font-family: POPPINS;padding-right: 10px;font-weight: 400;width: 75%;margin-left: 10px;'}), 
                                                                       lookup_expr='icontains')
    price = django_filters.RangeFilter(widget=MultiWidget(widgets=[forms.NumberInput(attrs={'class':'form-control',
                                                                                            'placeholder':'MIN',
                                                                                            'style':'width: 30%;margin-left: 10px;font-size: 15px;font-family: POPPINS;'}),
                                                                   forms.NumberInput(attrs={'class':'form-control',
                                                                                            'placeholder':'MAX',
                                                                                            'style':'width: 30%;margin-left: 38px;font-size: 15px;font-family: POPPINS;'})]))
    class Meta:
        model= Job
        fields = {
            'status',
